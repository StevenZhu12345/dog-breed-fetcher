...
public class Main {
    ...
    public static int getNumberOfSubBreeds(String breed, BreedFetcher breedFetcher) {
        ...
        if (breedFetcher == null) throw new IllegalArgumentException("breedFetcher must not be null");
        if (breed == null || breed.isBlank()) return 0;  // treat as no sub-breeds

        try {
            List<String> subs = breedFetcher.getSubBreeds(breed);
            return (subs == null) ? 0 : subs.size();
        } catch (BreedFetcher.BreedNotFoundException e) {
            // The test expects 0 for invalid breed / fetch failure
            return 0;
        }
    }
}